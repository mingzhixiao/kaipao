...
export class SaveManager {
    ...
    try {
      ...
      if (!raw) {
        ...
        if (legacy) {
          ...
          return { ...this.getDefaultData(), ...old, scrap: old.scrap || 0 };
        }
  ...
  recordStageClear(stageId, scrapEarned) {
    ...
    if (stageId >= (this.data.unlockedStage || 1)) {
      this.data.unlockedStage = Math.min(8, stageId + 1);
    }
  ...
  addScrap(amount) {
    this.data.scrap = (this.data.scrap || 0) + Math.max(0, Math.round(amount));
    this.save();
  ...
  spendScrap(amount) {
    ...
    if ((this.data.scrap || 0) < amount) return false;
  ...
  setRuneLevel(id, level) {
    if (!this.data.runeLevels) this.data.runeLevels = {};
    this.data.runeLevels[id] = level;
    this.save();
  }
