...
export class Game {
  constructor() {
    ...
    this.skills = {
      rocket: {
        level: 0, cooldown: GAME_CONFIG.skills.rocket.cooldown, timer: 3.0,
        damage: GAME_CONFIG.skills.rocket.damage, radius: GAME_CONFIG.skills.rocket.radius,
        burnDuration: GAME_CONFIG.skills.rocket.burnDuration, burnDps: GAME_CONFIG.skills.rocket.burnDps
      },
      truck: {
        level: 0, cooldown: GAME_CONFIG.skills.truck.cooldown, timer: 4.0,
        damage: GAME_CONFIG.skills.truck.damage, knockback: GAME_CONFIG.skills.truck.knockback,
        speed: GAME_CONFIG.skills.truck.speed, width: GAME_CONFIG.skills.truck.width, height: GAME_CONFIG.skills.truck.height
      },
      freeze: {
        level: 0, cooldown: GAME_CONFIG.skills.freeze.cooldown, timer: 5.0,
        duration: GAME_CONFIG.skills.freeze.duration, activeTimer: 0,
        damagePerTick: GAME_CONFIG.skills.freeze.damagePerTick, slowRatio: GAME_CONFIG.skills.freeze.slowRatio,
        range: GAME_CONFIG.skills.freeze.range, coneAngle: GAME_CONFIG.skills.freeze.coneAngle
      }
    };
    this.synergies = { thermalEngine: false, teslaCoil: false, fortressEmp: false, truckInferno: false, cryoShatter: false };
    this.bullets = []; this.enemies = []; this.gems = []; this.particles = []; this.damageTexts = [];
  ...
  initPools() {
    this.bulletPool = new ObjectPool(() => ({
      active: false, x: 0, y: 0, vx: 0, vy: 0, damage: 0, isCrit: false, pierceLeft: 1, radius: 4, life: 2.0
    }), (b) => { b.x = 0; b.y = 0; b.vx = 0; b.vy = 0; b.damage = 0; b.isCrit = false; b.pierceLeft = 1; }, 80, 200);
    this.enemyPool = new ObjectPool(() => ({
      active: false, type: 'runner', isBoss: false, x: 0, y: 0, vx: 0, vy: 0, hp: 100, maxHp: 100,
      speed: 80, radius: 18, color: '#38ef7d', expVal: 8, physicalResistance: 0, attackPower: 30, attackCooldown: 1.0, attackTimer: 0,
      ...
      e.burnTimer = 0; e.burnDps = 0; e.freezeTimer = 0; e.freezeFactor = 1.0; e.physicalResistance = 0;
    ...
    this.gemPool = new ObjectPool(() => ({
      active: false, x: 0, y: 0, vx: 0, vy: 0, val: 5, radius: 5, timer: 0, color: '#00f0ff'
    }), (g) => { g.timer = 0; g.vx = 0; g.vy = 0; }, 60, 150);
    this.textPool = new ObjectPool(() => ({
      active: false, text: '', x: 0, y: 0, vy: -45, color: '#fff', fontSize: 14, scale: 1.4, targetScale: 1.0,
      life: 0.7, maxLife: 0.7, isSpecial: false, isCrit: false
    }), null, 50, 120);
    this.hitRingPool = new ObjectPool(() => ({
      active: false, x: 0, y: 0, radius: 10, maxRadius: 28, life: 0.22, maxLife: 0.22, color: '#00f0ff'
    }), null, 30, 80);
  }
  ...
  resetGame() {
    ...
    this.burnZones.length = 0; this.activeTrucks.length = 0; this.iceSpikes.length = 0;
    this.teslaArcs.length = 0; this.shockwaves.length = 0; this.muzzleFlash = 0;
  ...
  getRoadBounds(y) {
    ...
    const roadWidth = right - left; const center = this.width / 2;
    return { left, right, center, roadWidth, leftLane: left + roadWidth * 0.25, centerLane: center, rightLane: left + roadWidth * 0.75 };
  }
  ...
  spawnParticles(x, y, color, count, type = 'spark') {
    ...
    const active = this.particles.length;
    if (active >= MAX_ACTIVE) return;
    if (active + count > MAX_ACTIVE) count = MAX_ACTIVE - active;
  ...
  spawnHitRing(x, y, color = '#ff0055', radius = 25) {
    ...
    hr.active = true; hr.x = x; hr.y = y; hr.radius = 8; hr.maxRadius = radius;
    hr.life = hr.maxLife = 0.24; hr.color = color;
  ...
  update(dt) {
    ...
    if (this.emergencyOverlay) {
      if (hpRatio <= 0.3) {
        ...
        if (this.heartbeatTimer >= 1.0) { this.heartbeatTimer = 0; sound.playHeartbeat(); }
      } else {
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      if (e.freezeTimer > 0) { e.freezeTimer -= dt; e.freezeFactor = this.skills.freeze.slowRatio; }
      ...
      if (e.isBoss) {
        ...
        if (e.stompTimer <= 0) {
          ...
          this.feedback.addTrauma(0.42); sound.playExplosion();
          this.shockwaves.push({ active: true, x: e.x, y: e.y + e.radius * 0.5, radius: 10, maxRadius: 240, life: 0.5, maxLife: 0.5, color: '#ff2a5f' });
        }
      }
    ...
    for (let i = 0; i < this.gems.length; i++) {
      ...
      const dx = this.hero.x - g.x, dy = this.hero.y - g.y;
      const dist = Math.hypot(dx, dy);
      if (dist < this.hero.magnetRange && dist > 0.01) {
        const pull = Math.min(1, (this.hero.magnetRange - dist) / this.hero.magnetRange) * 480 * dt;
        g.x += (dx / dist) * pull; g.y += (dy / dist) * pull;
      }
      if (dist < 28) {
        g.active = false; this.gemPool.release(g);
        this.gainExp(g.val || 5);
        if (sound.playGemPickup) sound.playGemPickup();
      }
    }
    ...
    const viewMargin = 40;
    const vw = this.width + viewMargin, vh = this.height + viewMargin;
    for (let i = 0; i < this.particles.length; i++) {
      ...
      if (p.life <= 0 || p.x < -viewMargin || p.y < -viewMargin || p.x > vw || p.y > vh) {
        p.active = false; this.particlePool.release(p);
      }
    }
  ...
  damageFortress(dmg, sourceEnemy = null) {
    ...
    if (this.fortress.shield > 0) {
      ...
      if (this.fortress.shield < 0) {
        ...
        if (this.synergies.fortressEmp) this.synergySystem.triggerShieldBreakEmp();
      }
    } else {
